        ...
        match movement {
            ...
            Movement::Down => {
                if self
                    .moving_obj_coords
                    .iter()
                    .any(|x| self.objects[x.1 - 1][x.0] == Obj::StoppedRock)
                {
    ...
    fn simulate(
        ...
        for run in 1..num_runs + 1 {
            ...
            for row in new_moving_coords.iter() {
                self.moving_obj_coords
                    .push((row.0, row.1 + self.highest_rock + 4));
            }
...
fn main() {
    ...
    let initial_objs: Vec<Vec<Obj>> = vec![
        vec![Obj::StoppedRock; 7],
        vec![Obj::Air; 7],
        vec![Obj::Air; 7],
        vec![Obj::Air; 7],
    ];
    let mut cave = Cave {
        objects: initial_objs,
        moving_obj_coords: Vec::new(),
        width: 7,
        highest_rock: 0,
    };
